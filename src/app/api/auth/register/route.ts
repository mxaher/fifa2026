...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      if (config?.fromEmail && (config?.apiKey || (config?.mailjetApiKey && config?.mailjetSecretKey))) {
        ...
        const html =
          ...
          '<a href="' + verifyLink + '" style="display:inline-block;padding:14px 40px;background:linear-gradient(135deg,#FFD700,#FFA000);color:#000;border-radius:8px;text-decoration:none;font-weight:bold;font-size:16px;">تأكيد البريد الإلكتروني</a></div>' +
          '<p style="font-size:13px;color:#888;">إذا لم يعمل الرابط، يمكنك نسخ الرابط التالي ولصقه في المتصفح:</p>' +
          '<p style="font-size:12px;color:#4FC3F7;word-break:break-all;background:rgba(0,0,0,0.3);padding:10px;border-radius:6px;">' + verifyLink + '</p></div>' +
    ...
    // No email configured or send failed — auto-verify the user
    try {
