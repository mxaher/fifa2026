        ...
        <div className="w-full lg:w-96 lg:order-1 flex-shrink-0">
          <div className="rounded-2xl p-6 sticky top-4" style={{ background: 'rgba(10,22,40,0.85)', backdropFilter: 'blur(20px)', border: '1px solid var(--border-color)' }}>
            <div className="flex justify-center mb-4">
              <img src="/almarshd-logo.png" alt="المرشد القابضة" className="h-14 w-auto" />
            </div>
            ...
            <div className="text-center">
              ...
              <div className="rounded-xl p-4 mb-4" style={{ background: 'var(--bg-primary)', border: '1px solid var(--border-color)' }}>
                ...
                <div className="text-xs mt-2" style={{ color: 'var(--text-muted)' }}>
                  {new Date(selectedMatch.kickoff).toLocaleDateString('ar-SA', { calendar: 'gregory', weekday: 'long', month: 'long', day: 'numeric', timeZone: 'Asia/Riyadh' })}
                  {' • '}
                  {new Date(selectedMatch.kickoff).toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Riyadh' })}
                </div>
              </div>
    ...
    <div className="space-y-4">
      ...
      {tab === 'config' && (
        <div className="rounded-xl p-5 space-y-4" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)' }}>
          ...
          {config?.lastSentAt && (
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
              آخر إرسال: {new Date(config.lastSentAt).toLocaleString('ar-SA', { timeZone: 'Asia/Riyadh' })}
            </p>
          )}
        </div>
      )}
                  ...
                  {logs.map((log: any) => (
                    <tr key={log.id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                      <td className="px-3 py-2" style={{ color: 'var(--text-secondary)' }}>
                        {log.createdAt ? new Date(log.createdAt).toLocaleString('ar-SA', { timeZone: 'Asia/Riyadh' }) : '-'}
                      </td>
